-- Fix vista_deudas: permitir saldos negativos (a favor) en ventas para que el saldo total sea correcto
BEGIN;
...
pagos_cuenta_corriente AS (
  ...
  WHERE p.venta_id IS NULL OR v.estado_pago = 'cancelado'
),
todas_deudas AS (
...
SELECT
  ...
  COALESCE(SUM(CASE WHEN td.saldo > 0 THEN td.saldo ELSE 0 END), 0)::DECIMAL(12,2) AS total_deuda_bruta,
  COALESCE(SUM(td.saldo), 0)::DECIMAL(12,2) AS saldo_total,
