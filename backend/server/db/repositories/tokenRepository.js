...
async function saveRefreshToken({ user_id, token_hash, jti, user_agent, ip, expires_at }) {
  await query(
    `INSERT INTO auth_refresh_tokens(user_id, token, jti, user_agent, ip, expires_at)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [user_id, token_hash, jti, user_agent || null, ip || null, expires_at]
  );
}

async function revokeRefreshToken(token_hash) {
  await query(`UPDATE auth_refresh_tokens SET revoked_at = NOW() WHERE token = $1`, [token_hash]);
}
