...
async function crearPago({ venta_id, cliente_id, monto, fecha, metodo = 'efectivo' }) {
  return withTransaction(async (client) => {
    ...
    await client.query(
      `INSERT INTO pagos(venta_id, cliente_id, monto, fecha, metodo)
       VALUES ($1, $2, $3, $4, $5)`,
      [venta_id, cliente_id, monto, fecha || new Date(), metodo]
    );
...
async function listarPagos({ venta_id, cliente_id, limit = 100, offset = 0 } = {}) {
  ...
  const { rows } = await query(
    `SELECT id, venta_id, cliente_id, monto::float AS monto, fecha, metodo
       FROM pagos
